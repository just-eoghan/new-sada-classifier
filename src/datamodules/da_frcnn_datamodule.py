...
class DaFrcnnDatamodule(LightningDataModule):
    """
    ...
    def __init__(
        ...
        source_annotations_file: str = "",
        target_data_dir: str = "data/",
        target_annotations_file: str = "",
    ...
    def setup(self, stage: Optional[str] = None):
        if not self.data_train_source and not self.data_train_target and not self.data_val:
            self.data_train_source = DaFrcnnDataset(
                self.hparams.source_data_dir,
                self.hparams.source_annotations_file,
            ...
            self.data_train_target = DaFrcnnDataset(
                self.hparams.target_data_dir,
                self.hparams.target_annotations_file,
                remove_images_without_annotations=True,
                is_source=False,
                transform=self.transforms
            )
            ...
            self.data_val = DaFrcnnDataset(
                ...
                self.hparams.val_annotations_file,
                remove_images_without_annotations=True,
                # Not sure about this for val might need to be set true depending.
                is_source=False,
    ...
    def train_dataloader(self):
        return [
            DataLoader(
                ...
                shuffle=(not self.hparams.dispair), # no shuffle for dispair
            ...
            DataLoader(
                ...
                num_workers=self.hparams.num_workers,
                pin_memory=self.hparams.pin_memory,
                shuffle=(not self.hparams.dispair),
                # https://github.com/pytorch/vision/issues/2624#issuecomment-681811444
                collate_fn=self.collater,
            )
        ]
