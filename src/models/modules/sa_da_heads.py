...
from src.models.modules.da_loss.gradient_scalar_layer import GradientScalarLayer
from src.models.modules.da_loss.pooler import LevelMapper
from src.models.modules.da_loss.da_loss import SaDALossComputation
...
class SaDomainAdaptationModule(torch.nn.Module):
    ...
    def __init__(self, in_channels, consit_weight, img_grl_weight, ins_grl_weight):
        ...
        self.imghead = DAImgHead(256) # in_channels # 256 * 4 1024
    ...
    def forward(self, img_features, da_ins_feature, da_ins_labels, da_proposals, targets=None):
        ...
        # x = draw_graph(model=self.inshead, input_data=[ins_grl_fea, levels], 
        #     save_graph=True, directory="/home/eoghan/Code/streetseek-da",
        #     depth=1, strict=True, mode='train', roll=True,)
        ...
        # x = draw_graph(model=self.imghead, input_data=[img_grl_fea], 
        # save_graph=True, directory="/home/eoghan/Code/streetseek-da", graph_dir='LR',
        #     depth=1, strict=True, mode='train', roll=True,)

        # make_dot(da_img_features[0], params=dict(self.imghead.named_parameters()))
